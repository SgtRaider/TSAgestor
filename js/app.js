...
(function () {
  ...
  function buildTsaRow(t, opts) {
    ...
    const editBtn = (t._source === 'kml')
      ? ` <button type="button" class="btn-kml-edit" data-id="${escapeHTML(t.id)}" aria-label="Editar TSA KML ${escapeHTML(t.name || t.id)}" title="Editar altitud y validez de esta TSA KML">✎</button>`
      : '';
    tr.innerHTML = `
      <td class="col-check">
  ...
  function wireKMLEditor() {
    ...
    // Tecla Escape cierra el modal cuando esta abierto. Solo escuchamos
    // si el modal no esta hidden para no robar Escape a otros modales.
    document.addEventListener('keydown', (e) => {
      if (e.key !== 'Escape') return;
      const m = $('#kml-edit-modal');
